...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 17 16:50:44 2026
...
class ImageDataset(Dataset):
    def __init__(self, img_tensors, num_list, target_list, orig_img_num):
        self.img_tensors = img_tensors
        self.num_list = num_list
        self.target_list = target_list
        self.orig_img_num = orig_img_num
    ...
    def __getitem__(self, index):
        ...
        orig_num = self.orig_img_num[index]

        return img, num, target, orig_num
...
def evaluate (model, dataloader):
    ...
    all_orig_num = []
    with torch.no_grad():
       for image, num, label, orig_num in dataloader:
           outputs, _, _ = model(image)
           all_labels.extend(label.numpy())
           all_predictions.extend(outputs.numpy())
           all_num.extend(num.numpy())
           all_orig_num.extend(orig_num.numpy())
    return all_labels, all_predictions, all_num, all_orig_num
...
def get_feature (model, dataloader):
    ...
    feat_num = []
    with torch.no_grad():
       for image, num, label, _ in dataloader:
